...
import infinite_talk.InfiniteTalk.wan as wan
from infinite_talk.InfiniteTalk.wan.configs import SIZE_CONFIGS, SUPPORTED_SIZES, WAN_CONFIGS
from infinite_talk.InfiniteTalk.wan.utils.utils import str2bool, is_video, split_wav_librosa
from infinite_talk.InfiniteTalk.wan.utils.multitalk_utils import save_video_ffmpeg
from infinite_talk.InfiniteTalk.kokoro import KPipeline
from transformers import Wav2Vec2FeatureExtractor
from infinite_talk.InfiniteTalk.src.audio_analysis.wav2vec2 import Wav2Vec2Model
from infinite_talk.InfiniteTalk.wan.utils.segvideo import shot_detect
...
def generate(args) -> bytes:
    ...
    if rank == 0:
        ...
        # Save video to in-memory buffer using ffmpeg
        import tempfile
        with tempfile.NamedTemporaryFile(suffix=".mp4") as temp_video:
            save_video_ffmpeg(sum_video, temp_video.name, [input_data['video_audio']], high_quality_save=False)
            final_video_path = temp_video.name + ".mp4"
            with open(final_video_path, "rb") as f:
                video_bytes = f.read()
            # Optionally, clean up the generated file
            os.remove(final_video_path)
        logging.info("Returning video bytes output.")
        return base64.b64encode(video_bytes)
    else:
        # Always return bytes, never None
        logging.warning("Empty bytes will be returned as rank != 0")
