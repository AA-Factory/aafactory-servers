...
def main(*func_args, **func_kwargs):
    ...
    with torch.inference_mode(), ctx:
        ...
        for q in range(args.queue_size):
            vhs_videoinfo_212 = vhs_videoinfo.get_video_info(video_info=["13", 3])

            floatconstant_28 = floatconstant.get_value(
                value=get_value_at_index(vhs_videoinfo_212, 2)
            )

            mathexpressionpysssss_21 = mathexpressionpysssss.evaluate(
                expression=parse_arg(args.expression31),
            ...
            vhs_loadvideo_13 = vhs_loadvideo.load_video(
                ...
                skip_first_frames=parse_arg(args.skip_first_frames35),
                select_every_nth=parse_arg(args.select_every_nth36),
                format="Wan",
            )

            imageresizekjv2_33 = imageresizekjv2.resize(
                width=get_value_at_index(intconstant_32, 0),
                height=parse_arg(args.height37),
                upscale_method=parse_arg(args.upscale_method38),
                keep_proportion=parse_arg(args.keep_proportion39),
                pad_color=parse_arg(args.pad_color40),
                crop_position=parse_arg(args.crop_position41),
                divisible_by=parse_arg(args.divisible_by42),
                device="cpu",
                per_batch=0,
                image=get_value_at_index(vhs_loadvideo_13, 0),
                unique_id=3173124992677765822,
            )

            generateframesbycount_9 = generateframesbycount.r(
                frame_count=parse_arg(args.frame_count43),
                revert=parse_arg(args.revert44),
            ...
            imageresizekjv2_63 = imageresizekjv2.resize(
                ...
                height=parse_arg(args.height45),
                upscale_method=parse_arg(args.upscale_method46),
                keep_proportion=parse_arg(args.keep_proportion47),
                pad_color=parse_arg(args.pad_color48),
                crop_position=parse_arg(args.crop_position49),
                divisible_by=parse_arg(args.divisible_by50),
                device="cpu",
                per_batch=0,
                image=get_value_at_index(loadimage_64, 0),
                unique_id=9349985941159609702,
            )
            ...
            pixelperfectresolution_58 = pixelperfectresolution.execute(
                ...
                resize_mode=parse_arg(args.resize_mode51),
            ...
            vhs_videocombine_52 = vhs_videocombine.combine_video(
                frame_rate=parse_arg(args.frame_rate52),
                loop_count=parse_arg(args.loop_count53),
                filename_prefix=parse_arg(args.filename_prefix54),
                format=parse_arg(args.format55),
                pix_fmt="yuv420p",
                crf=19,
                save_metadata=True,
                trim_to_audio=False,
                pingpong=parse_arg(args.pingpong56),
                save_output=parse_arg(args.save_output57),
                images=get_value_at_index(dwpreprocessor_53, 0),
            )

            facemaskfromposekeypoints_198 = facemaskfromposekeypoints.createmask(
                person_index=parse_arg(args.person_index58),
                pose_kps=get_value_at_index(dwpreprocessor_53, 1),
            )

            imagecropbymaskandresize_56 = imagecropbymaskandresize.crop(
                base_resolution=parse_arg(args.base_resolution59),
                padding=parse_arg(args.padding60),
                min_crop_resolution=parse_arg(args.min_crop_resolution61),
                max_crop_resolution=parse_arg(args.max_crop_resolution62),
                image=get_value_at_index(imageresizekjv2_33, 0),
                mask=get_value_at_index(facemaskfromposekeypoints_198, 0),
            )

            wanvideomodelloader_75 = wanvideomodelloader.loadmodel(
                model=parse_arg(args.model63),
                base_precision=parse_arg(args.base_precision64),
                quantization=parse_arg(args.quantization65),
                load_device=parse_arg(args.load_device66),
            ...
            wanvideoclipvisionencode_87 = wanvideoclipvisionencode.process(
                strength_1=parse_arg(args.strength_167),
                strength_2=parse_arg(args.strength_268),
                crop=parse_arg(args.crop69),
                combine_embeds=parse_arg(args.combine_embeds70),
                force_offload=parse_arg(args.force_offload71),
                tiles=0,
                ratio=0.5,
            ...
            mathexpressionpysssss_144 = mathexpressionpysssss.evaluate(
                expression=parse_arg(args.expression72),
            ...
            wanvideoanimateembeds_88 = wanvideoanimateembeds.process(
                ...
                force_offload=parse_arg(args.force_offload73),
                frame_window_size=get_value_at_index(mathexpressionpysssss_144, 0),
                colormatch=parse_arg(args.colormatch74),
                pose_strength=parse_arg(args.pose_strength75),
                face_strength=parse_arg(args.face_strength76),
                tiled_vae=False,
            ...
            wanvideosampler_90 = wanvideosampler.process(
                steps=parse_arg(args.steps77),
                cfg=parse_arg(args.cfg78),
                shift=parse_arg(args.shift79),
                seed=parse_arg(args.seed80),
                force_offload=parse_arg(args.force_offload81),
                scheduler=parse_arg(args.scheduler82),
                riflex_freq_index=parse_arg(args.riflex_freq_index83),
                denoise_strength=1,
                batched_cfg="",
                rope_function="comfy",
                start_step=0,
            ...
            wanvideodecode_91 = wanvideodecode.decode(
                enable_vae_tiling=parse_arg(args.enable_vae_tiling84),
                tile_x=parse_arg(args.tile_x85),
                tile_y=parse_arg(args.tile_y86),
                tile_stride_x=parse_arg(args.tile_stride_x87),
                tile_stride_y=parse_arg(args.tile_stride_y88),
            ...
            vhs_videocombine_106 = vhs_videocombine.combine_video(
                ...
                loop_count=parse_arg(args.loop_count89),
                filename_prefix=parse_arg(args.filename_prefix90),
                format=parse_arg(args.format91),
                pix_fmt="yuv420p",
                crf=19,
                save_metadata=True,
                trim_to_audio=False,
                pingpong=parse_arg(args.pingpong92),
                save_output=parse_arg(args.save_output93),
            ...
            vhs_videocombine_141 = vhs_videocombine.combine_video(
                frame_rate=parse_arg(args.frame_rate94),
                loop_count=parse_arg(args.loop_count95),
                filename_prefix=parse_arg(args.filename_prefix96),
                format=parse_arg(args.format97),
                pix_fmt="yuv420p",
                crf=19,
                save_metadata=False,
                trim_to_audio=False,
                pingpong=parse_arg(args.pingpong98),
                save_output=parse_arg(args.save_output99),
            ...
            rife_vfi_171 = rife_vfi.vfi(
                ckpt_name=parse_arg(args.ckpt_name100),
                clear_cache_after_n_frames=get_value_at_index(cm_floattoint_176, 0),
                multiplier=get_value_at_index(intconstant_29, 0),
                fast_mode=parse_arg(args.fast_mode101),
                ensemble=parse_arg(args.ensemble102),
                scale_factor=parse_arg(args.scale_factor103),
            ...
            vhs_videocombine_178 = vhs_videocombine.combine_video(
                ...
                loop_count=parse_arg(args.loop_count105),
                filename_prefix=parse_arg(args.filename_prefix106),
                format=parse_arg(args.format107),
                pix_fmt="yuv420p",
                crf=19,
                save_metadata=True,
                trim_to_audio=False,
                pingpong=parse_arg(args.pingpong108),
                save_output=parse_arg(args.save_output109),
